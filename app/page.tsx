...
const PHONE_NUMBER_DISPLAY = "+1 (201) 277-8569";
const PHONE_NUMBER_PLAIN = "12012778569";
...
const copy = {
	es: {
		...
		hero: {
			...
			ctaLabel: "WhatsApp",
			ctaNotice:
				"Meta baneo nuestro numero anterior. Escribenos a este numero nuevo por favor.",
			ctaHint:
				"Escribe una vez. Quedas registrado para recibir nuevas actualizaciones oficiales cuando ONPE publique cambios.",
		},
		metric: {
			label: "Usuarios",
			caption:
				"registrados ya siguen por WhatsApp el ultimo conteo oficial publicado por ONPE.",
			fallback:
				"El ultimo conteo oficial de ONPE, entregado automaticamente por WhatsApp.",
		},
		steps: {
			label: "Como funciona",
			items: [
		...
		footer: {
			note: "Fuente: ONPE.",
			detail:
				"Un bot para seguir las ultimas elecciones en Peru sin estar revisando la web a cada rato.",
			website: "Website",
	...
	en: {
		...
		hero: {
			...
			ctaLabel: "WhatsApp",
			ctaNotice:
				"Meta banned our previous number, message this new number please.",
			ctaHint:
				"Send one message. You stay registered for future official updates whenever ONPE publishes a new count.",
		},
		metric: {
			label: "Users",
			caption:
				"registered users already follow the latest official ONPE count through WhatsApp.",
			fallback:
				"The latest official ONPE count, delivered automatically through WhatsApp.",
		},
		steps: {
			label: "How it works",
			items: [
		...
		footer: {
			note: "Source: ONPE.",
			detail:
				"A bot for following the latest Peru election count without refreshing the web all day.",
			website: "Website",
			...
			<div className="page-shell-inner">
				<header className="relative z-10 w-full">
					<div className="mx-auto flex w-full max-w-[44rem] items-center justify-between px-5 py-5 md:px-6 md:py-6">
						...
						<nav
							className="flex items-center gap-1 text-[13px] text-white/62"
				...
				<section
					className="relative z-10 mx-auto flex w-full max-w-[44rem] flex-1 px-5 pb-20 pt-8 md:px-6 md:pt-12"
					lang={lang}
				>
					<div className="w-full space-y-10 md:space-y-12">
						<div className="space-y-4">
							<p className="text-[13px] leading-7 text-white/42">
								{content.hero.eyebrow}
							</p>
							<h1 className="max-w-[34rem] text-[clamp(2.9rem,10vw,5rem)] leading-[0.94] tracking-[-0.07em] text-white/96">
								{content.hero.title}
							</h1>
							<p className="max-w-[34rem] text-[15px] leading-7 text-white/62">
								{content.hero.description}
							</p>
						</div>

						<div className="space-y-3">
							<p className="text-[13px] leading-7 text-white/34">
							...
							<p className="max-w-[34rem] text-[13px] leading-6 text-white/62">
								{content.hero.ctaNotice}
							</p>
						...
						<div className="space-y-7 pt-2">
							<div className="grid grid-cols-[72px_1fr] gap-4 md:grid-cols-[84px_1fr]">
								<p className="text-[13px] leading-9 text-white/34">
								...
								<div className="space-y-2">
									{totalUsers !== null ? (
										<>
											...
											<p className="max-w-[28rem] text-[15px] leading-7 text-white/62">
												{content.metric.caption}
											</p>
										</>
									) : (
										<p className="max-w-[28rem] text-[15px] leading-7 text-white/62">
											{content.metric.fallback}
										</p>
									)}
								</div>
							</div>
							...
							<div className="grid grid-cols-[72px_1fr] gap-4 md:grid-cols-[84px_1fr]">
								...
								<ul className="space-y-2.5">
									{content.steps.items.map((item, index) => (
										<li
											key={item}
											className="grid grid-cols-[24px_1fr] gap-3 text-[15px] leading-7 text-white/62"
										>
