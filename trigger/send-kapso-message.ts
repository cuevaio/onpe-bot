...
import { kapsoSendQueue, kapsoWebhookSendQueue } from "@/trigger/queues";
...
async function sendKapsoPayload(
	payload: z.infer<typeof sendKapsoMessagePayloadSchema>,
) {
...
export const sendKapsoMessage = schemaTask({
	...
	catchError: async ({ payload, error }) => {
		...
		logger.info(
			"Skipping retries for Kapso message outside WhatsApp session window",
			{
				type: payload.type,
				to: payload.to,
